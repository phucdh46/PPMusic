package com.dhp.musicplayer.core.network.innertube
...
import com.dhp.musicplayer.core.network.innertube.model.Context.Companion.clientDefaultWeb
import com.dhp.musicplayer.core.network.innertube.model.GridRenderer
import com.dhp.musicplayer.core.network.innertube.model.MoodAndGenres
import com.dhp.musicplayer.core.network.innertube.model.MusicCarouselShelfRenderer
...
import com.dhp.musicplayer.core.network.innertube.model.MusicTwoRowItemRenderer
...
import com.dhp.musicplayer.core.network.innertube.utils.findSectionByTitle
import com.dhp.musicplayer.core.network.innertube.utils.from
import com.dhp.musicplayer.core.network.innertube.utils.getConfig
...
class InnertubeApiService(val context: android.content.Context) {
    ...
    suspend fun relatedPage(body: NextBody) = runCatchingNonCancellable {
        ...
        val response = client.post(browse) {
            setBody(
                BrowseBody(browseId = browseId).copy(
                context = Context(client = clientDefaultWeb.copy(visitorData = config.visitorData))
            ))
...
//        mask("contents.singleColumnBrowseResultsRenderer.tabs.tabRenderer.content.sectionListRenderer.contents(musicPlaylistShelfRenderer(continuations,contents.$musicResponsiveListItemRendererMask),gridRenderer(continuations,items.$musicTwoRowItemRendererMask))")
    ...
    suspend fun moodAndGenres(): Result<List<MoodAndGenres>> = runCatching {
        val response = client.post(browse) {
            setBody(
                BrowseBody(browseId = "FEmusic_moods_and_genres").copy(
                    context = Context(client = clientDefaultWeb.copy(visitorData = config.visitorData))
                    ))
        }
            .body<BrowseResponse>()
    ...
    suspend fun browse(browseId: String, params: String?): Result<BrowseResult> = runCatching {
        val response = client.post(browse) {
            setBody(
                BrowseBody(browseId = browseId, params = params).copy(
                    context = Context(client = clientDefaultWeb.copy(visitorData = config.visitorData))
                    ))
        }.body<BrowseResponse>()
        BrowseResult(
